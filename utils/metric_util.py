...
def compute_cls_metric(y_true, y_pred):
    ...
    for i in range(num_labels):
        ...
        negative_count = np.sum(label == 0)
        loguru.logger.info(f"Label {i}: Positive samples = {positive_count}, Negative samples = {negative_count}")
...
def compute_cls_metric_tensor(y_true: Tensor, y_pred: Tensor):
    ...
    roc_auc: torch.Tensor = torch.mean(roc_list).cuda()
